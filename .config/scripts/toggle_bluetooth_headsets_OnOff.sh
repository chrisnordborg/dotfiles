#!/bin/bash

log() {
    echo "[LOG] $1"
}

notify() {
    notify-send "Bluetooth Audio" "$1"
}

declare -A headsets=(
  ["A8:F5:E1:86:64:FA"]="Shokz OpenRun Mini"
  ["84:D3:52:0B:D4:A3"]="JBL Live Pro 2"
  ["14:3F:A6:C2:9D:9A"]="Sony WF-1000XM4"
  ["30:50:75:F2:8B:F8"]="Jabra Evolve 75"
  ["C0:28:8D:AA:99:55"]="Jaybird Vista 2"
)

ordered_macs=("A8:F5:E1:86:64:FA" "84:D3:52:0B:D4:A3" "14:3F:A6:C2:9D:9A" "30:50:75:F2:8B:F8" "C0:28:8D:AA:99:55")

log "Unblocking Bluetooth with rfkill"
rfkill unblock bluetooth

# Wait for bluetoothctl to become responsive
log "Checking bluetoothctl..."
for i in {1..5}; do
    if timeout 1 bluetoothctl show &>/dev/null; then
        log "bluetoothctl is ready"
        break
    fi
    if [ "$i" -eq 5 ]; then
        log "bluetoothctl not responding, restarting service..."
        sudo systemctl restart bluetooth
        sleep 2
    else
        log "Waiting for bluetoothctl... ($i)"
        sleep 1
    fi
done

# Power on controller
echo -e "power on" | bluetoothctl &>/dev/null
sleep 1

# Disconnect if any headset is already connected
for mac in "${ordered_macs[@]}"; do
    if bluetoothctl info "$mac" | grep -q 'Connected: yes'; then
        bluetoothctl disconnect "$mac" &>/dev/null
        notify "Disconnected: ${headsets[$mac]}"
        echo "Disconnected: ${headsets[$mac]}"
        exit 0
    fi
done

# Try to connect
for mac in "${ordered_macs[@]}"; do
    log "Trying to connect to ${headsets[$mac]} ($mac)"
    bluetoothctl <<EOF &>/dev/null
trust $mac
connect $mac
EOF

    # Wait for sink to appear
    for i in {1..10}; do
        sink=$(pactl list short sinks | grep -i "${mac//:/_}" | awk '{print $2}')
        if [ -n "$sink" ]; then
            pactl set-default-sink "$sink"
            notify "Connected: ${headsets[$mac]}"
            echo "Connected: ${headsets[$mac]}"
            exit 0
        fi
        sleep 1
    done

    log "No sink found for ${headsets[$mac]}"
done

notify "No Bluetooth headset connected"
echo "No Bluetooth headset connected"
exit 1

